...
const SEED_CONFIG = {
  ...
  ORDERS: 20,
  REVIEWS_PERCENTAGE: 0.6, // 60% of orders will have reviews
  CART_ITEMS: 15,
  WISHLIST_ITEMS: 30,
};
...
const generateJewelryVariants = (
  ...
  if (isJewelry) {
    ...
    if (productName.includes("Ring")) {
      ...
      const colors = getRandomElements(
        COLORS,
      ...
      colors.forEach((color) => {
        variants.push({
          productId,
          sku: `${faker.string.alphanumeric(8).toUpperCase()}-${color.replace(
            /\s+/g,
            ""
          )}`,
          name: color,
    ...
    sizes.forEach((size) => {
      colors.forEach((color) => {
        variants.push({
          ...
          sku: `${faker.string
            .alphanumeric(8)
            .toUpperCase()}-${size}-${color.replace(/\s+/g, "")}`,
          name: `${size} - ${color}`,
...
const generateJewelryAttributes = () => {
  ...
  attributes.push(
    ...
    {
      name: "Carat Weight",
      value: `${faker.number.float({ min: 0.1, max: 5, multipleOf: 0.1 })}ct`,
    },
    {
      name: "Setting Type",
      value: getRandomElement(["Prong", "Bezel", "Channel", "Pave"]),
    }
  );
  ...
  if (Math.random() > 0.7) {
    attributes.push({
      name: "Chain Length",
      value: `${faker.number.int({ min: 14, max: 30 })}"`,
    });
  }
...
const generateClothingAttributes = () => {
  ...
  attributes.push(
    ...
    {
      name: "Fit",
      value: getRandomElement(["Regular", "Slim", "Loose", "Fitted"]),
    },
  ...
  if (Math.random() > 0.6) {
    attributes.push({
      name: "Pattern",
      value: getRandomElement(["Solid", "Striped", "Floral", "Geometric"]),
    });
  }
...
async function main() {
  ...
  const categoryData = [
    ...
    // Clothing categories
    {
      name: "Dresses",
      slug: "dresses",
      description: "Fashionable dresses for women",
      isJewelry: false,
    },
    {
      name: "Tops",
    ...
    },
    {
      name: "Activewear",
      slug: "activewear",
      description: "Sporty and comfortable clothing",
      isJewelry: false,
    },
  ];
  ...
  for (let i = 0; i < SEED_CONFIG.USERS; i++) {
    const user = await prisma.user.create({
      data: {
        ...
        role:
          i < SEED_CONFIG.SELLERS
            ? UserRole.SELLER
            : i < SEED_CONFIG.SELLERS + 3
            ? UserRole.ADMIN
            : UserRole.CUSTOMER,
  ...
  for (const product of products) {
    if (faker.datatype.boolean({ probability: 0.8 })) {
      // 80% of products have variants
      const variants = generateJewelryVariants(
        product.id,
        product.productName,
        product.isJewelry
      );
  ...
  for (let i = 0; i < SEED_CONFIG.CART_ITEMS; i++) {
    ...
    const product = getRandomElement(
  ...
  for (let i = 0; i < SEED_CONFIG.ORDERS; i++) {
    ...
    const shippingAmount = faker.number.float({
      min: 0,
      max: 25,
      multipleOf: 0.01,
    });
  ...
  for (const order of orders) {
    ...
    const sellerProducts = products.filter(
      (p) => p.sellerId === order.sellerId && p.status === ProductStatus.ACTIVE
    );
    ...
    for (let i = 0; i < numItems; i++) {
      ...
      const variant = faker.helpers.maybe(
        () =>
          getRandomElement(
            allVariants.filter((v) => v.productId === product.id)
          ),
        { probability: 0.6 }
      );
  ...
  for (const order of shippedOrders) {
    ...
    const deliveredAt =
      order.status === OrderStatus.DELIVERED
        ? faker.date.between({ from: shippedAt, to: new Date() })
        : null;
  ...
  const reviewCount = Math.floor(
    deliveredOrders.length * SEED_CONFIG.REVIEWS_PERCENTAGE
  );
